...
@section('title')
    كشف المردودية الجماعي
@endsection
@section('content-title')
    <h1> كشف المردودية الجماعي
    </h1>
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="#">كشف مردودية</a></li>
            ...
            <table class="table table-striped ">
                <thead>
                    <tr>
                        ...
                        <th class="col-md-3" scope="col">العنوان</th>
                        <th class="col-md-6" scope="col">العمليات</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($re_megrations as $re_megration)
                        <tr>
                            <td>{{ $re_megration->YEAR }}</td>
                            <td>{{ Mlibrary::getRendament($re_megration->TRIMESTRE) }}</td>
                            <td>
                                {{ $re_megration->TITLE ?? '/' }}
                            </td>
                                ...
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu  dropdown-menu-bottom" role="menu" style="">
                                        ...
                                        <form class="dropdown-item" style="padding: 0;" target="_black"
                                                    action="{{ route('rendement-global-print') }}" method="post">
                                                    @csrf
                                                    <input type="hidden" name="ID_MEGRATION_RE"
                                                        value="{{ $re_megration->ID_MEGRATION_RE }}">
