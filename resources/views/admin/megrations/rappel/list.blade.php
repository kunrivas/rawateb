...
@section('content-title')
    <h1> تسير الحزم المخلفات </h1>
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="#">المخلفات </a></li>
...
@section('contents')
    ...
    <div class="row mb-2">
        <div class="col"></div>
        <div class="col-2">
            <a class="btn btn-success " href="{{ route('admin-megration-rappel-create') }}">إضافة حزمة جديدة</a>
        </div>
    </div>
    <div class="row">
        <div class="col">
                ...
                <tbody>
                    @foreach ($megrations as $megration)
                        <tr>
                            ...
                            <td>
                                {{-- STATUS تم تنفيذها وتغلق البوطون
                                    RUN يتم التنفيذ او لم يتم التنفيذ --}}
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu" role="menu" style="">
                                        @if ($megration->STATUS == 0)
                                            <form action="{{ route('admin-megration-rappel-run') }}" method="post"
                                                id="megrationForm">
                                                @csrf
                                                <input type="hidden" name="ID_MEGRATION"
                                                    value="{{ $megration->ID_MEGRATION_RA }}">
                                                <button type="submit" id="submitBtn"
                                                    class="dropdown-item text-success font-weight-bold"
                                                    href="#">تنفيذ</button>
                                            </form>
                                            <form action="{{ route('admin-megration-rappel-delete') }}" method="post"
                                        ...
                                        @else
                                            {{--   <a class="btn btn-success " href="{{ route('admin-megration-rappel-stat' , ['ID_MEGRATION_RA' => $megration->ID_MEGRATION_RA]) }}">إضافة حزمة جديدة</a>
                                       --}} <button class="dropdown-item show-info-btn ml-2"
                                                data-id="{{ $megration->ID_MEGRATION_RA }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p><strong>رمز الحزمة : </strong> <span id="ID_MEGRATION_RA"></span></p>
...
@section('js')
    <script>
        $(document).on('click', '.show-info-btn', function() {
            const ID_MEGRATION_RA = $(this).data('id'); // Fetch the ID_MEGRATION from the button
            //alert(ID_MEGRATION_RA);
            $.ajax({
                url: `/rawateb/manager/megration/rappel/${ID_MEGRATION_RA}/stat`, // Use the route
                method: 'GET',
                success: function(data) {
                    // Populate the modal fields with the data
                    $('#infoModal #ID_MEGRATION_RA').text(data.ID_MEGRATION_RA);
    ...
    {{--  <script defer>
        $(".run").click(function(evt) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('admin-megration-rappel-run') }}",
                data: {
