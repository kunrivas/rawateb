...
@section('content-title')
    <h1> أرشيف التحديثات </h1>
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="#">تسير الحزم</a></li>
    <li class="breadcrumb-item"><a href="#"> تحديث بيانات الموظفين</a></li>
@endsection
...
@section('contents')
    ...
    <div class="row mb-2">
        <div class="col"></div>
        <div class="col-2">
            <a class="btn btn-success " href="{{ route('admin-megration-employees-datas-create') }}">إضافة تحديث جديدة</a>
        </div>
    </div>
    <div class="row">
        <div class="col">
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu" role="menu" style="">
                                        @if ($megration->STATUS == 0)
                                            <form action="{{ route('admin-megration-employees-datas-run') }}" method="post"
                                                id="megrationForm">
                                                @csrf
                                       ...
                                       --}}
                                            <form action="{{ route('admin-megration-employees-datas-delete') }}" method="get"
                                                onsubmit="return confirm('هل أنت متأكد من أنك تريد حذف هذا التحديث؟');">
                                                <input type="hidden" name="ID_MEGRATION"
                                                    value="{{ $megration->ID_MEGRATION }}">
                                                <button type="submit"
                                        ...
                                        @else
                                            {{--    <button class="dropdown-item show-info-btn ml-2"
                                                ...
                                                إحصائيات التحديث </button>
                                            <div class="dropdown-divider"></div> --}}
                                            <form action="{{ route('admin-megration-employees-datas-delete') }}"
                                                method="get"
                                                onsubmit="return confirm('هل أنت متأكد من أنك تريد حذف هذا التحديث؟');">
                                                <input type="hidden" name="ID_MEGRATION"
                                                    value="{{ $megration->ID_MEGRATION }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p><strong>رمز التحديث : </strong> <span id="ID_MEGRATION"></span></p>
                    <p><strong>عنوان التحديث : </strong> <span id="TITLE"></span></p>
                    <p><strong>سنة التحديث : </strong> <span id="YEAR"></span></p>
                    <p><strong>lot التحديث : </strong> <span id="LOT"></span></p>
...
@section('js')
    <script>
        $(document).on('click', '.show-info-btn', function() {
            ...
            //alert(ID_MEGRATION);
            $.ajax({
                url: `/rawateb/manager/megration/tamadres/${ID_MEGRATION}/stat`, // Use the route
                method: 'GET',
