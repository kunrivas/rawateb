...
@section('title')
    تسير الحزم المردودية
@endsection
@section('content-title')
    <h1> تسير الحزم المردودية </h1>
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="#">المردودية</a></li>
...
@section('contents')
    ...
    <div class="row mb-2">
        <div class="col"></div>
        <div class="col-2">
            <a class="btn btn-success " href="{{ route('admin-megration-rendement-create') }}">إضافة حزمة جديدة</a>
        </div>
    </div>
    <div class="row">
        <div class="col">
                ...
                <tbody>
                    @foreach ($megrations as $megration)
                        <tr>
                            ...
                            <td>
                                {{ Mlibrary::getRendament($megration->TRIMESTRE) }}
                            </td>
                            <td>
                                {{ $megration->TITLE ?? '/' }}
                            </td>
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu" role="menu" style="">
                                        @if ($megration->STATUS == 0)
                                            <form action="{{ route('admin-megration-rendement-run') }}" method="post"
                                                id="megrationForm">
                                                @csrf
                                                <input type="hidden" name="ID_MEGRATION"
                                                    value="{{ $megration->ID_MEGRATION_RE }}">
                                                <button type="submit" id="submitBtn"
                                                    class="dropdown-item text-success font-weight-bold"
                                                    href="#">تنفيذ</button>
                                            </form>
                                            {{--   <button type="button" class="btn-run dropdown-item text-success font-weight-bold" megration="{{ $megration->ID_MEGRATION_RE }}">تنفيذ</button>

                                       --}}
                                            <form action="{{ route('admin-megration-rendement-delete') }}" method="post"
                                        ...
                                        @else
                                            {{-- <a class="btn btn-success "
                                                href="{{ route('admin-megration-rendement-stat', ['ID_MEGRATION_RE' => $megration->ID_MEGRATION_RE]) }}">إضافة
                                                حزمة جديدة</a> --}}
                                            <button class="dropdown-item show-info-btn ml-2"
                                                data-id="{{ $megration->ID_MEGRATION_RE }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p><strong>رمز الحزمة : </strong> <span id="ID_MEGRATION_RE"></span></p>
                    <p><strong>ثلاثي الحزمة : </strong> <span id="TRIMESTRE"></span></p>
...
@section('js')
    <script>
        $(document).on('click', '.show-info-btn', function() {
            const ID_MEGRATION_RE = $(this).data('id'); // Fetch the ID_MEGRATION from the button
            //alert(ID_MEGRATION_RE);
            $.ajax({
                url: `/rawateb/manager/megration/rendement/${ID_MEGRATION_RE}/stat`, // Use the route
                method: 'GET',
                success: function(data) {
                    // Populate the modal fields with the data
                    $('#infoModal #ID_MEGRATION_RE').text(data.ID_MEGRATION_RE);
                    $('#infoModal #TRIMESTRE').text(data.TRIMESTRE);
