...
@section('content-title')
    <h1> تسير حزم التمدرس </h1>
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="#">التمدرس</a></li>
...
@section('contents')
    ...
    <div class="row mb-2">
        <div class="col"></div>
        <div class="col-2">
            <a class="btn btn-success " href="{{ route('admin-megration-tamadres-create') }}">إضافة حزمة جديدة</a>
        </div>
    </div>
    <div class="row">
        <div class="col">
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu" role="menu" style="">
                                        @if ($megration->STATUS == 0)
                                            <form action="{{ route('admin-megration-tamadres-run') }}" method="post"
                                                id="megrationForm">
                                                @csrf
                                                <input type="hidden" name="ID_MEGRATION"
                                                    value="{{ $megration->ID_MEGRATION_TA }}">
                                                <button type="submit" id="submitBtn"
                                                    class="dropdown-item text-success font-weight-bold"
                                                    href="#">تنفيذ</button>
                                            </form>
                                            {{--   <button type="button" class="btn-run dropdown-item text-success font-weight-bold" megration="{{ $megration->ID_MEGRATION_TA }}">تنفيذ</button>

                                       --}}
                                            <form action="{{ route('admin-megration-tamadres-delete') }}" method="post"
                                                onsubmit="return confirm('هل أنت متأكد من أنك تريد حذف هذه الحزمة؟');">
                                                @csrf
                                                <input type="hidden" name="ID_MEGRATION"
                                                    value="{{ $megration->ID_MEGRATION_TA }}">
                                        ...
                                        @else
                                            <button class="dropdown-item show-info-btn ml-2"
                                                data-id="{{ $megration->ID_MEGRATION_TA }}">
                                                إحصائيات الحزمة </button>
                                            <div class="dropdown-divider"></div>
                                            <form action="{{ route('admin-megration-tamadres-delete') }}" method="get"
                                                onsubmit="return confirm('هل أنت متأكد من أنك تريد حذف هذه الحزمة؟');">
                                                <input type="hidden" name="ID_MEGRATION"
                                                    value="{{ $megration->ID_MEGRATION_TA }}">
                                                <button type="submit"
                                                    class="dropdown-item text-danger font-weight-bold">حذف</button>
                                            </form>
                                        @endif
                                    </div>
                                </div>
     ...
     <div class="modal-dialog">
         <div class="modal-content">
             ...
             <div class="modal-body">
                 <p><strong>رمز الحزمة : </strong> <span id="ID_MEGRATION_TA"></span></p>
...
<script>
    $(document).on('click', '.show-info-btn', function() {
        const ID_MEGRATION_TA = $(this).data('id'); // Fetch the ID_MEGRATION from the button
        //alert(ID_MEGRATION_TA);
        $.ajax({
            url: `/rawateb/manager/megration/tamadres/${ID_MEGRATION_TA}/stat`, // Use the route
            method: 'GET',
            success: function(data) {
                // Populate the modal fields with the data
                $('#infoModal #ID_MEGRATION_TA').text(data.ID_MEGRATION_TA);
