...
@section('title')
    تسير الحزم المخلفات مردودية
@endsection
@section('content-title')
    <h1> تسير الحزم المخلفات مردودية</h1>
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="#">المخلفات مردودية</a></li>
...
@section('contents')
    ...
    <div class="row mb-2">
        <div class="col"></div>
        <div class="col-2">
            <a class="btn btn-success " href="{{ route('admin-megration-rappel-prime-create') }}">إضافة حزمة جديدة</a>
        </div>
    </div>
    <div class="row">
        <div class="col">
                            ...
                            <td>
                                <div class="btn-group">
                                    ...
                                    <button type="button" class="btn btn-outline-success btn-sm dropdown-toggle"
                                        data-toggle="dropdown" aria-expanded="false" {{--  @if ($megration->STATUS == 1) disabled @endif --}}>
                                        <span class="sr-only"> </span>
                                    </button>
                                    <div class="dropdown-menu" role="menu" style="">
                                        @if ($megration->STATUS == 0)
                                            <form action="{{ route('admin-megration-rappel-prime-run') }}" method="post"
                                                id="megrationForm">
                                                @csrf
                                                <input type="hidden" name="ID_MEGRATION"
                                                    value="{{ $megration->ID_MEGRATION_RA_RE }}">
                                           ...
                                           --}}
                                            <form action="{{ route('admin-megration-rappel-prime-delete') }}" method="post"
                                                onsubmit="return confirm('هل أنت متأكد من أنك تريد حذف هذا العنوان؟');">
                                                @csrf
                                                <input type="hidden" name="ID_MEGRATION"
                                                    value="{{ $megration->ID_MEGRATION_RA_RE }}">
                                        ...
                                        @else
                                           {{--  <a class="btn btn-success "
                                                href="{{ route('admin-megration-rappel-prime-stat', ['ID_MEGRATION_RA_RE' => $megration->ID_MEGRATION_RA_RE]) }}">إضافة
                                                حزمة جديدة</a> --}}
                                            <button class="dropdown-item show-info-btn ml-2"
                                                data-id="{{ $megration->ID_MEGRATION_RA_RE }}">
                                                إحصائيات الحزمة </button>
                                            <div class="dropdown-divider"></div>
                                            <form action="{{ route('admin-megration-rappel-prime-delete') }}"
                                                method="post"
                                                onsubmit="return confirm('هل أنت متأكد من أنك تريد حذف هذا العنوان؟');">
                                                @csrf
                                                <input type="hidden" name="ID_MEGRATION"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p><strong>رمز الحزمة : </strong> <span id="ID_MEGRATION_RA_RE"></span></p>
                    <p><strong>عنوان الحزمة : </strong> <span id="TITLE"></span></p>
                    <p><strong>سنة الحزمة : </strong> <span id="YEAR"></span></p>
                    <p><strong>LOT الحزمة : </strong> <span id="LOT"></span></p>
...
<script>
    $(document).on('click', '.show-info-btn', function() {
        const ID_MEGRATION_RA_RE = $(this).data('id'); // Fetch the ID_MEGRATION from the button
        //alert(ID_MEGRATION_RA_RE);
        $.ajax({
            url: `/rawateb/manager/megration/rappel-prime/${ID_MEGRATION_RA_RE}/stat`, // Use the route
            method: 'GET',
            success: function(data) {
                // Populate the modal fields with the data
                $('#infoModal #ID_MEGRATION_RA_RE').text(data.ID_MEGRATION_RA_RE);
                $('#infoModal #TITLE').text(data.TITLE);
    ...
    {{--  <script defer>
        $(".run").click(function(evt) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('admin-megration-rappel-prime-run') }}",
                data: {
