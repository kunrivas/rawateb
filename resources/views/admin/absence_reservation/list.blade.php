...
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('admin-absence') }}">تسير الغيابات </a></li>
@endsection
...
@section('contents')
    <div class="row mb-2">
        <div class="col"></div>
        <div class="col-2">
            <a class="btn btn-success " href="{{ route('admin-absence-create') }}">إضافة شهر جديد</a>
        </div>
    </div>
    <div class="row">
        <div class="col">
                                ...
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu" role="menu" style="">
                                        ...
                                        <a class="dropdown-item font-weight-bold"
                                            href="{{ route('admin-absence-employees-list', $absence_reservation->absence_reservation_id) }} ">معاينة</a>
                                        {{-- btn of "طباعة" --}}
                                        <a class="dropdown-item font-weight-bold"
                                            href="{{ route('admin-absence-print', $absence_reservation->absence_reservation_id) }} ">طباعة</a>

                                        {{-- btn of "فتح/غلق" --}}
                                        <form action=" {{ route('admin-absence-status') }} " method="post">
                                            @csrf
                                            <input type="hidden" name="id"
                                                value="{{ $absence_reservation->absence_reservation_id }}">
                                        ...
                                        {{-- btn of "delete"
                                        <form action="{{ route('admin-absence-delete') }}" method="post">
                                            @csrf
                                            <input type="hidden" name="id"
                                                value="{{ $absence_reservation->absence_reservation_id }}">
                                        ...
                                        @foreach ($adms as $adm)
                                            @if ($adm->ADM != '0')
                                                <form class="dropdown-item" style="padding: 0;" target="_black"
                                                    action="{{ route('admin-absence-export-excel') }}" method="post">
                                                    @csrf
                                                    <input type="hidden" name="absence_reservation_id"
                                                        value="{{ $absence_reservation->absence_reservation_id }}">
