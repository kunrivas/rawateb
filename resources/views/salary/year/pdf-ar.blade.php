...
<body>
    <div align="center">
        <table style="width:920px;" class="font16">
            ...
            <tr>
                <td> <span>المؤسسة: </span><span>@if (is_null(session()->get("establishment"))) /@else{{ session()->get("establishment")->estab_ar_name }}  @endif</span></td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td class="info"><span>الوظيفة :</span><span>
                      @if (is_null($salary_single->fonction)) / ({{ $salary_single->ADM }}) @else {{ $salary_single->fonction->LIBTABA }} ({{ $salary_single->ADM }}) @endif</span></td> 
            </tr>
            <tr>
                <td class="info" colspan="3"><span>الإسم :</span> <span> {{ $employee->PRENOMA }}</span></td>
                <td class="info"><span>الصنف : </span><span>{{ $salary_single->CATEG }}</span></td>
            </tr>
            <tr>
                <td class="info" colspan="3"> <span>الحالة العائلية :</span>
                    <span>{{ $salary_single->SITFAM }}</span>
                </td>
                <td class="info"><span>الدرجة : </span><span>{{ $salary_single->ECH }}</span></td>
            </tr>
            <tr class="info-last">
