...
<body>
    @if ($rendement_reservations_adms->count() > 0)
        ...
        @foreach ($rendement_reservations_adms as $adm => $rendement_reservations_employees)
            <div align="center">
                <table style="width:920px;" class="font16">
                    ...
                    <tr>
                        <td colspan="4"> <span>المؤسسة: </span><span>{{ $establishment->estab_ar_name }}</span></td>
                        <td style="margin-right:50px " rowspan="2"> <img
                                src=" https://api.qrserver.com/v1/create-qr-code/?size=80x80&data={rend_id={{ $rendement_reservations_statistic->id }}}"
                                alt="" srcset=""></td>
                    </tr>
                    <tr>
                ...
                <table class="case">
                    ...
                    <tbody>
                        ...
                        @if ($rendement_reservations_employees->count() > 0)
                            @foreach ($rendement_reservations_employees as $key => $rendement_reservations_employee)
                                <tr>
                                    ...
                                    <td style="width: 30%;">{{ $rendement_reservations_employee->employee->NOMA }}
                                        {{ $rendement_reservations_employee->employee->PRENOMA }}</td>
                                    <td style="width: 7%;">
                                        {{ $rendement_reservations_employee->employee->fonction->CATEG ?? '/' }}</td>
                                    <td style="width: 7%;">
        ...
        @endforeach
    @else
        <h3 style="text-align: center">ليس هنالك نقاط محجوزة</h3>
    @endif
</body>
