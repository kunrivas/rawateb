...
<head>
    ...
    <style>
        ...
        .masked-pin {
            -webkit-text-security: disc !important; /* For Chrome/Safari/Edge */
            text-security: disc !important;         /* For others */
        }
        .pin-gap {
            width: 40px;
            height: 50px;
            text-align: center;
            font-size: 24px;
            /* This masks the 'tel' input like a password */
...
<body>
    <div class="card">
        ...
        <form action="{{ route('pin.check') }}" method="POST" id="pin-form">
            @csrf
            <!-- The "Gaps" -->
            ...
            <!-- Hidden field to combine them for your Controller -->
            <input type="hidden" name="pin" id="full-pin">

            <button type="submit" style="margin-top: 20px;">التحقق من الهوية</button>
        </form>
    ...
    <script>
        ...
        document.querySelectorAll('.pin-gap').forEach((input, index, inputs) => {
            input.addEventListener('input', (e) => {
                ...
                // Combine all values into the hidden field
                let combined = "";
        ...
        // Clean up on refresh/back button
        window.onpageshow = () => {
            document.querySelectorAll('.pin-gap').forEach((input, index, inputs) => {
                input.value = '';
            });
